...
var Parallax = {
				...
				for (var att in endPoints) {
					...
					if (!isNaN(moveVal) && !isNaN(diff)) {
						//if (dir == "down") {
							...
							if(startval>endval)
							{
								finalVal= startval+(startval * (percent / 100)) + endPoints[att].suffix;
								// elem.style[att] 
							}else{
								finalVal=startval-(startval * (percent / 100)) + endPoints[att].suffix;
								// elem.style[att] = 
	...
	moveIn : function(holderId, percent, dir) {
		console.log(holderId, percent, dir);
		for (var name in this.movers) {
			var obj = this.movers[name];
			if (obj.holder == holderId) {
				var elem = document.getElementById(this.movers[name].elem);
						...
						if(startval>endval)
							{
								...
								finalVal = (endval+moveVal) ;
								//elem.style[att] = endval+(moveVal) +endPoints[att].suffix;
							}else{
								finalVal=(startval+moveVal); 
								console.log(finalVal,endval);
								if(finalVal>endval)finalVal=endval;
								//elem.style[att] = endval-(moveVal) + endPoints[att].suffix;
							}
						// if (dir != "up") {
							finalVal+= endPoints[att].suffix;
							if (!window.TweenLite) {
								elem.style[att] = finalVal;
							} else {
								css[att] = finalVal;
